...
/**
 * Cache generique des donnees de base (athlete, summary, settings) pour un
 * rendu instantane de l'Accueil et des entetes. Meme store, cle dediee.
 */
export async function readBaseCache(userId) {
  try {
    const db = await openDb();
    return await new Promise((resolve) => {
      const tx = db.transaction(STORE, "readonly");
      const req = tx.objectStore(STORE).get(`base:${userId || "anon"}`);
      req.onsuccess = () => {
        const val = req.result;
        if (!val || !val.data) { resolve(null); return; }
        if (Date.now() - (val.cachedAt || 0) > MAX_AGE_MS) { resolve(null); return; }
        resolve(val.data);
      };
      req.onerror = () => resolve(null);
    });
...
export async function writeBaseCache(userId, data) {
  try {
    const db = await openDb();
    await new Promise((resolve) => {
      const tx = db.transaction(STORE, "readwrite");
      tx.objectStore(STORE).put({ data, cachedAt: Date.now() }, `base:${userId || "anon"}`);
      tx.oncomplete = () => resolve();
      tx.onerror = () => resolve();
    });
  } catch {
