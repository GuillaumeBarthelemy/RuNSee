...
export function groupActivitiesByUser(activities = []) {
...
export function detectProviderDuplicatesForUserActivities(activities = [], minScore = 70) {
...
export async function listProviderDuplicateCandidates(appUserId = "") {
  const where = {
    isMerged: false,
...
export async function detectProviderActivityDuplicates({ appUserId = "", minScore = 70 } = {}) {
  const activities = await listProviderDuplicateCandidates(appUserId);
  const groups = groupActivitiesByUser(activities);
  const duplicates = [];

  for (const [userId, userActivities] of groups.entries()) {
    const userDuplicates = detectProviderDuplicatesForUserActivities(userActivities, minScore);
